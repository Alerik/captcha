...
namespace APIHelper
{
	public class APIFunction
	{
		...
		internal APIFunction(API _api, string _Path, List<APIArgument> _Arguments, List<Table> _Dependencies)
		{
			this.api = _api;
			this.Path = _Path;
		...
		void GenerateClientCode()
		{
			...
			creator.GenerateCode(Dependencies[0], Name, Path + ".php", Arguments.Select(a => new Column(a.Identifier, a.DBType)).ToList(), Dependencies[0].ExposedColumns);
			Console.End();	
  }
		...
		void GenerateServerCode()
		{
			...
			if (!Dependencies[0].Defined)
				ConsoleHelper.Warn("The dependency {0} is undefined", Dependencies[0].Name);
			creator.CreateCode(Dependencies[0], Name, Arguments.Select(a => new Column(a.Identifier, a.DBType)).ToList(), Dependencies[0].InternalColumns, Dependencies[0].ExposedColumns);
			Console.End();	
  }
	}
}
